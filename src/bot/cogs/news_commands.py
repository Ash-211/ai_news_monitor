...
def fetch_articles(
    ...
    """
    Unified query function supporting category and/or date filters, with pagination offset.
    """
    session = get_session()
    try:
...
def get_fallback_date(category: Optional[str] = None) -> Optional[date]:
    """
    Finds the date of the most recent article, optionally filtered by category.
    """
    session = get_session()
    try:
        query = session.query(Article)
...
def _fetch_articles_for_date(
    target_date: date,
    category: Optional[str] = None,
    limit: int = MAX_EMBED_ARTICLES,
) -> List[Article]:
    """
    Query articles whose published_at falls on *target_date*.
    Maintained as a thin wrapper for backward-compatibility (e.g. for scheduler cog).
    """
    return fetch_articles(category=category, target_date=target_date, offset=0, limit=limit)
...
class NewsPaginationView(discord.ui.View):
    ...
    def __init__(
        ...
        super().__init__(timeout=None)  # Infinite duration
    ...
    async def show_more_callback(self, interaction: discord.Interaction) -> None:
        ...
        # Fetch next batch (offset is self.offset)
        limit = 6
        
        category_filter = None
        date_filter = None
        
        if self.command_type == "daily":
            date_filter = self.search_date
        elif self.command_type == "category":
            category_filter = self.category
            date_filter = self.search_date
        elif self.command_type == "search":
            category_filter = self.search_tag
            date_filter = self.search_date
        ...
        for i in range(display_count):
            ...
            if is_last and has_more:
                # Attach Show More button to the 5th article
                ...
                # Standard view (only Read Full Article button)
...
# ════════════════════════════════════════════════════════════════════════════
#  Category Select Menu (for /newscategories)
# ════════════════════════════════════════════════════════════════════════════

class CategorySelect(discord.ui.Select):
    """Dropdown that lists every distinct article category."""
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        test_articles = fetch_articles(category=chosen, target_date=target_date, limit=1)
        
        fallback_msg = ""
        if not test_articles:
            fallback = get_fallback_date(category=chosen)
            if fallback:
        ...
        batch = fetch_articles(category=chosen, target_date=target_date, offset=0, limit=6)
...
class NewsCommands(commands.Cog):
    ...
    async def news(
        ...
        # Fetch page 1 (limit 6)
        batch = fetch_articles(category=tag, target_date=target_date, offset=0, limit=6)
